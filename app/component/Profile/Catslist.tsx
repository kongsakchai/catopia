                ...
                <div className='container flex flex-col items-center justify-center gap-8'>
                    <div className='container flex flex-col'>
                        {mockupKitten.map((kitten) => (
                            <div key={kitten.id} className='flex p-4 justify-between items-start'>
                                <div className='flex items-start gap-4'>
                                    <Image src={kitten.img_url} width={50} height={50} alt='Your kitten img' style={{ border: "1px solid var(--Primary, #3FA8D0)", borderRadius: '88px' }} />
                                    <div className='flex flex-col items-start'>
                                        <h2 className='text-black01 text-base not-italic font-normal leading-6'>{kitten.name}</h2>
                                        <h3 className='text-textfield text-xs not-italic font-normal leading-5'>อัพเดทล่าสุด : {kitten.last_update}</h3>
                                    </div>
                                </div>
                                <button type='button'>
                                    <Image src="/aboutcat-btn.svg" width={24} height={24} alt='About cats'></Image>
                                </button>
                            </div>
                        ))}
                    </div>
